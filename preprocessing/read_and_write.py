...
This file reads data from S3, preprocesses the .gz files into a dataframe, drops papers without abstracts, 
filters by number of citations, filters by relevant tags, and stores a reparitioned dataframe as a parquet
file into S3 so it can be loaded into S3 
'''
...
def read_and_store_files(bucket_name):
    '''
    ...
    for gz_file in bucket.objects.filter(Prefix="corpus"):
        ...
        if df_init_switch == False: 
            df = spark.read.text("s3a://{0}/{1}".format(bucket_name, gz_file.key)).repartition(16)
            ...
            df_being_read = clean_df(df_being_read)
            #df = df.union(df_being_read) # adds the rows of df_being_read to the existing df
        '''
    return
...
# already run 
#store_in_s3(df)
read_and_store_files(orig_bucket_name)
#single_file_name = "corpus-2019-01-31/s2-corpus-00.gz"
#df = spark.read.text("s3a://{0}/{1}".format(orig_bucket_name, single_file_name))
#print()
#df = clean_df(df)
#print()
#store_in_s3(df)
...
def read_from_s3():
	...
	'''
	df = spark.read.load("s3a://preprocessed-open-research-corpus/full_dataframe/part-00000-29442892-0dda-4a7d-b601-c4993d968e99-c000.snappy.parquet")
	return df
